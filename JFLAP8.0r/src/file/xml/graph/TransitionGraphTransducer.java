...
public class TransitionGraphTransducer extends StructureTransducer<TransitionGraph<? extends Transition<?>>>{
	...
	public TransitionGraph<? extends Transition<?>> fromStructureRoot(
		...
		Element cpMap_elem = XMLHelper.getChildArray(root, CTRL_POINT_MAP).get(0);
		Map<Point, Point2D> cpMap = cpMapTransducer.fromStructureRoot(cpMap_elem);
		...
		for(State s : a.getStates()){
			...
			for(State v : graph.adjacent(s)){
				Point2D ctrl = cpMap.get(new Point(s.getID(), v.getID()));
				graph.setControlPt(ctrl, s, v);
			}
		}
		return graph;
	}
	...
	public Element appendComponentsToRoot(Document doc,
		...
		AutomatonTransducer autoTrans = (AutomatonTransducer) TransducerFactory.getTransducerForStructure(auto);
