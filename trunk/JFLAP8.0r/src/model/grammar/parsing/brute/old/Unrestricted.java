...
public class Unrestricted {
	...
	public static Set<Symbol> smallerSymbols(Grammar grammar) {
		...
		Production[] prods = grammar.getProductionSet().toArray(new Production[0]);
		boolean added;
		do {
			added = false;
...
//		added[i] = g.isValidProduction(prods[i]);
		//TODO: Check if this actually affects things - Type Test should
			//select the correct parser, which functional dictates
			// that all productions are "valid"
		added[i] = true;
		for (i = 0; i < prods.length; i++)
			if (added[i])
