...
public enum JFLAPError {
	...
	INVALID_DEGREE("That is not a valid degree value", 
			"Invalid Input"),
			
	ILLEGAL_VAR_LOC("You cannot use a variable on the left side of the assignment operator!"+
					"\nPlease fix this and restart the simulation.", 
					"Illegal Variable Location!"),
					
	NO_INIT_STATE("Simulation requires an automaton\n with an initial state!", 
					"No Initial State"),
	TOO_MANY_STATES("There may be at most 26 states for conversion."),
	
	NO_INIT_STATE_BB("It appears that one of your building blocks, possibly nested, lacks an initial state.\n "+
            		"Please resolve this problem and restart the simulation.",
				  	"Missing Initial State"),
				  	
	PAREN_AS_TOKEN("Grammar has the ( or ) character(s), which are reserved in current mode.\n" +
					"See Preferences menu above to toggle this feature.",
					"Production Error"),
					
	NO_FOLLOW_SET("JFLAP failed to find a follow set."),
	
	VAR_NOT_FOUND("JFLAP failed to find a variable.  You may have used a variable on the " +
	...
	ALREADY_CNF("The grammar is already in Chomsky NF.",
					"Already in CNF"),
	...
	REQUIRES_TM_FILETYPE("Only Turing Machine files can be added as building blocks.", 
					"Wrong File Type"),
					
	INSUF_INPUT_TM("Input file does not have enough input for all tapes", "File read error"),
	
	NEED_TRANS_TO_TRAP("You have to make transition to the trap state!"), 
	
	DFA_NO_LAMBDA("One can't have lambda in the DFA!", 
					"Improper terminal"), 
	
	NOT_NFA("This is not an NFA!",
					"Not an NFA"), 
					
	TRANS_FROM_FINAL("There are transitions from final states.  Please remove them or change " +
                    "\nthe preference in the \"Preferences\" menu in the JFLAP main menu.",
                    "Transitions From Final States"),
                    
    ONLY_FA_PDA_TM("This feature only works with Finite Automata, Pushdown Automata, and Turing Machines"),
    
    SAME_FILE_TYPE("Files must be of the same type"), 
    
    INCOMPLETE_CONVERSION("Conversion unfinished!  Objects to convert are highlighted.",
