...
import view.grammar.parsing.cyk.EmptySetCellEditor;
...
public class CYKParseTablePanel extends RunningView{
	...
	public CYKParseTablePanel(OldCYKParser parser){
		this(new OldCYKParseModel(parser));
	}
	
	public CYKParseTablePanel(OldCYKParseModel model) {
	...
	public void setInput(SymbolString input) {
		OldCYKParser parser = getModel().getParser();
		if(input == null){
			myTable.setModel(new OldCYKParseModel(parser));
			return;
		}
		
		OldCYKParseModel model = new OldCYKParseModel(parser, input);
	...
	public OldCYKParseModel getModel(){
		return (OldCYKParseModel) myTable.getModel();
	}
