...
/**
 * This transducer is the codec for {@link jflap.model.grammar.lsystem.LSystem} objects.
 * 
 * @author Thomas Finley
 */

public class LSystemTransducer implements Transducer<LSystem> {
	/**
	 ...
	 */
	private String readAxiom(Element root) {
		NodeList list = root.getElementsByTagName(AXIOM_NAME);
		if (list.getLength() < 1)
	...
	private Grammar readGrammar(Element root) {
		...
		NodeList list = root.getElementsByTagName(RULE_NAME);
		for (int i = 0; i < list.getLength(); i++) {
			Production[] p = createRules(list.item(i));
			for (int j = 0; j < p.length; j++)
				g.isValidProduction(p[j]);
		}
		return g;
	}
	 ...
	 */
	private Map readParameters(Element root) {
		Map p = new HashMap();
		NodeList list = root.getElementsByTagName(PARAMETER_NAME);
		for (int i = 0; i < list.getLength(); i++) {
	 ...
	 * @return the {@link jflap.model.grammar.lsystem.LSystem} instance
	 */
	public java.io.Serializable fromStructureRoot(Element root) {
		String axiom = readAxiom(root);
		Grammar rules = readGrammar(root);
		Map parameters = readParameters(root);
